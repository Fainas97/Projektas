...
if (!$e_group) {
    $session->msg("d", "Trūksta grupės ID.");
    redirect('group.php');
}
?>
<?php
if (isset($_POST['update'])) {
    ...
    if (empty($errors)) {
        ...
        if ($result && $db->affected_rows() === 1) {
            //sucess
            $session->msg('s', "Grupė atnaujinta! ");
            redirect('edit_group.php?id=' . (int)$e_group['id'], false);
        } else {
            //failed
            $session->msg('d', ' Atsiprašome, nepavyko atnaujinti grupės!');
            redirect('edit_group.php?id=' . (int)$e_group['id'], false);
        }
    } else {
    ...
    <form method="post" action="edit_group.php?id=<?php echo (int)$e_group['id']; ?>" class="clearfix">
        ...
        <div class="form-group">
            ...
            <select class="form-control" name="status">
                <option <?php if ($e_group['group_status'] === '1') echo 'selected="selected"'; ?> value="1"> Aktyvus
                </option>
                <option <?php if ($e_group['group_status'] === '0') echo 'selected="selected"'; ?> value="0">Neaktyvus
                </option>
            </select>
        </div>
